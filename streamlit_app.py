...
def get_client_shap(client_line):
    reponse = requests.get(
        API_DATAFRAME_URL, json={"data": client_line.to_json(default_handler=str)}
    )
...
def get_client_shap_plot(client_line, forme="waterfall"):
    reponse = requests.get(
        API_PLOT_URL + "/" + forme,
        json={"data": client_line.to_json(default_handler=str)},
    )
...
def show_client(sk_id, show):
    ...
    show.write(
        "Type de prêt : "
        + ("Comptant" if client_line["NAME_CONTRACT_TYPE"] == 0 else "Renouvelable")
    )
    ...
    show.markdown(
        "# Statut prédit : " + (":green[Accepté]" if statut == 0 else ":red[Refusé]")
    )
...
def main():
    ...
    st.set_page_config(
        page_title="Rapport d'analyse du statut client",
    ...
    sk_id = st.sidebar.text_input(
        "ID Client", placeholder="Entrez l'ID client (exemple : 100002)"
    )
    ...
    pages = {
        ...
        "personalised_graph": personalised_graph,
    }

    # Affiche le menu de sélection de page
    with st.sidebar:
        page = st.selectbox(
            "Mode d'analyse:", pages.keys(), format_func=lambda k: pages[k].description
        )
