...
def run(dataset, client_line, shap_df, shap_img):
    ...
    def determine_best_chart(data, mask_t0, client, alt=False, log=[False, False]):
        ...
        if num_cols == 1:
            ...
            if np.issubdtype(column_data.dtype, np.number) and nb_unique>2:
                ...
                if alt:
                    ...
                    plt.hist(data_t0, bins=50, color=COLOR_A, range=rge, alpha=0.5, label='Accepté', density=True, log=log)
                    plt.hist(data_not_t0, bins=50, color=COLOR_R, range=rge, alpha=0.5, label='Refusé', density=True, log=log)
            ...
            # Sinon, si elles sont catégorielles ou binaires
                ...
                plt.bar(positions_t0, counts_t0.reindex(categories, fill_value=0), width=0.35, label='Accepté', color=COLOR_A, log=log)
                plt.bar(positions_not_t0, counts_not_t0.reindex(categories, fill_value=0), width=0.35, label='Refusé', color=COLOR_R, log=log)
                
                plt.axvline(x=categories.index(client)+0.17, color=COLOR_C, linestyle='--', label='Client')
        ...
        # Cas où le tableau a deux colonnes
            ...
            # Graphique combiné
            plot = sns.jointplot(data=data_resample, x=x_col, y=y_col, hue=labels, palette={'Accepté':COLOR_A+'20', 'Refusé':COLOR_R+'40'})
            
            # Ajout des repères de l'emplacement du client
            plot.ax_joint.axvline(x=x_client, color=COLOR_C, linestyle='--', label='Client')
    ...
    if feat_2 not in list_cols:
        ...
        determine_best_chart(dataset[[feat_1, feat_2]], mask_t0, val_client, alt, [feat_1_log, feat_2_log])
